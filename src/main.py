...
def main():
    ...
    while True:
        ...
        if file_selection == "1":
            ...
            transactions_file = get_transactions_data(
                f"{os.path.join(os.path.dirname(__file__), os.pardir)}\\data" f"\\operations.json"
            )
            break
        elif file_selection == "2":
            print("Для обработки выбран CSV-файл.")
            transactions_file = get_csv_data(
                f"{os.path.join(os.path.dirname(__file__), os.pardir)}\\data" f"\\transactions.csv"
            )
            break
        elif file_selection == "3":
            print("Для обработки выбран XLSX-файл.")
            transactions_file = get_excel_data(
                f"{os.path.join(os.path.dirname(__file__), os.pardir)}\\data" f"\\transactions_excel.xlsx"
            )
            break
        else:
